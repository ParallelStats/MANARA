...
- The current rich adapter is MapLibre GL JS 6.6 using OpenFreeMap's keyless Liberty vector style. It requires no map account, token, or map environment variable. Provider place/POI symbols are suppressed inside the adapter so MANARA's own curated marker and label hierarchy remains dominant.
- A deliberate built-in fallback renderer preserves the same destination and hotspot controls while MapLibre initializes and when WebGL, style loading, or required network resources fail. Map availability is visual enhancement, not a routing dependency.
- World labels use MapLibre's collision-aware symbol placement when available and a shared priority/bounding-box resolver in fallback mode. City hotspot DOM markers use the same resolver with selected and playable locations receiving priority; markers remain present when a lower-priority label is hidden.
...
- `SpeechPlaybackPort`: plays human-recorded audio when supplied, otherwise attempts browser speech synthesis, and exposes cancellation.

The deterministic implementations are production-quality fallback adapters, not temporary UI conditionals. `GeminiConversationProvider` uses the server-only `@google/genai` SDK and GA Interactions API with `gemini-3.5-flash`, structured JSON output, `store: false`, a bounded timeout, cancellation, and normalized quota/network errors. Generated output is always stamped `needs_review`; a probable intent can map to progression only through one matching human-verified response option. Verified fixed dialogue always wins. The Guide consumes only MANARA evaluation output and remains a separate city-specific human companion, never a Gemini chat surface.

Browser speech recognition was selected instead of a persistent Gemini Live session for the competition path. It keeps the permanent API key server-side, avoids a long-lived raw-PCM/WebSocket lifecycle, and degrades immediately to typed or scripted input. Google documents ephemeral Live API tokens as Preview; Live remains a future replaceable adapter if its reliability and credential lifecycle become appropriate.
...
4. For typed or recognized speech, the browser may send one bounded transcript and scene context to `/api/conversation`. The server calls Gemini only when `GEMINI_API_KEY` is configured. An operation ID and abort signal reject duplicate or stale work.
