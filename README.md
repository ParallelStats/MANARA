...
Liberty was selected after production inspection because its land, water, streets, and regional boundaries remain readable inside MANARA's dark chrome. MANARA applies a dark regional palette with visible country boundaries while keeping city circles—not whole countries—as the exact selection targets. Provider city and POI labels are reduced so custom destination markers, selection labels, routes, and city hotspots stay visually dominant. OpenFreeMap/OpenStreetMap attribution remains visible in the map controls.
...
The microphone button uses the browser's Arabic speech-recognition capability (`ar-AE` in Abu Dhabi and `ar-EG` in Cairo). Tapping once begins listening; tapping again calls the browser's stop operation and submits the transcript captured so far. Browser support varies, and the browser or operating-system speech service may process audio according to its own policies. MANARA does not persist raw audio or recognized text. **Type instead** and scene-safe practice choices remain available. Character playback uses browser speech synthesis when available; generated voices are not described as authentically Emirati or Egyptian, and the playback port allows reviewed human recordings to override them later.
